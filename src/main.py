...
texttospeech_client = texttospeech.TextToSpeechAsyncClient()
bytes_per_sample = 2
...
async def synthesize_audio(text: str, sample_rate: int) -> bytes:
    """Synthesizes audio using the GCloud Text to Speech API."""
    synthesis_input = texttospeech.SynthesisInput(text=text)
    voice = texttospeech.VoiceSelectionParams(language_code=language, name=f"{language}-Wavenet-A")
    ...
    response = await texttospeech_client.synthesize_speech(input=synthesis_input, voice=voice, audio_config=audio_config)
...
async def synthesize(request: Request):
    ...
    try:
        async with asyncio.timeout(30):
            ...
            audio_buffer = await synthesize_audio(clean_text_for_tts(message['text']), message['sampleRate'])
